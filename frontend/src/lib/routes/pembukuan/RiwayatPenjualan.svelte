...
<script lang="ts">
  ...
  // Sorting
  let sortField = $state<'id' | 'created_ms' | 'total_harga_jual' | 'total_barang'>('created_ms');
...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-neutral-200/80 dark:border-neutral-800 pb-4">
    <div>
      ...
      <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
        Log transaksi invoice kasir, rincian barang per struk, dan opsi cetak ulang nota kasir.
      </p>
    </div>
  ...
  <!-- Summary Metric Cards (3 KPI Cards) -->
  <!--
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
    <div class="p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-[var(--bg-surface)] shadow-xs flex items-center justify-between">
      <div>
        <span class="text-[11px] font-medium text-neutral-500 uppercase tracking-wider">Total Transaksi</span>
        <div class="text-xl font-bold font-mono text-neutral-900 dark:text-neutral-100 tabular-nums mt-1">
          {formatNumber(totalTransaksi)} <span class="text-xs font-normal text-neutral-400">struk</span>
        </div>
      </div>
    ...
    <div class="p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-[var(--bg-surface)] shadow-xs flex items-center justify-between">
      <div>
        <span class="text-[11px] font-medium text-neutral-500 uppercase tracking-wider">Total Omzet Penjualan</span>
        <div class="text-xl font-bold font-mono text-neutral-900 dark:text-neutral-100 tabular-nums mt-1">
          {formatRupiah(totalOmzet)}
        </div>
      </div>
      <div class="w-9 h-9 rounded-md bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-500/20 flex items-center justify-center text-emerald-600 dark:text-emerald-400">
    ...
    <div class="p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-[var(--bg-surface)] shadow-xs flex items-center justify-between">
      <div>
        <span class="text-[11px] font-medium text-neutral-500 uppercase tracking-wider">Estimasi Laba Kotor</span>
        <div class="text-xl font-bold font-mono text-emerald-600 dark:text-emerald-400 tabular-nums mt-1">
    ...
    <div class="p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-[var(--bg-surface)] shadow-xs flex items-center justify-between">
      <div>
        <span class="text-[11px] font-medium text-neutral-500 uppercase tracking-wider">Rata-rata Transaksi</span>
        <div class="text-xl font-bold font-mono text-neutral-900 dark:text-neutral-100 tabular-nums mt-1">
          {formatRupiah(avgTransaksi)}
        </div>
      </div>
      <div class="w-9 h-9 rounded-md bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-300">
  ...
  <div class="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-[var(--bg-surface)] shadow-xs overflow-hidden">
      ...
      <table class="w-full text-left text-xs border-collapse">
        <thead>
          <tr class="border-b border-neutral-200 dark:border-neutral-800 bg-[var(--bg-subtle)]/60 text-neutral-500 font-mono text-[11px]">
            ...
            <th class="px-4 py-3 font-medium text-center cursor-pointer select-none" onclick={() => toggleSort('total_barang')}>
              <div class="flex items-center justify-center gap-1">
                <span>ITEM</span>
                <ArrowUpDown class="w-3 h-3 text-neutral-400" />
              </div>
            </th>
            <th class="px-4 py-3 font-medium text-right cursor-pointer select-none" onclick={() => toggleSort('total_harga_jual')}>
              <div class="flex items-center justify-end gap-1">
                <span>TOTAL BELANJA</span>
                <ArrowUpDown class="w-3 h-3 text-neutral-400" />
              </div>
            </th>
            <th class="px-4 py-3 font-medium text-right">LABA KOTOR</th>
          ...
          {:else}
            {#each filteredList as item (item.id)}
              ...
              <tr class="hover:bg-neutral-50/60 dark:hover:bg-neutral-900/40 transition-colors">
                ...
                <!-- Total Barang -->
                <td class="px-4 py-3 text-center font-mono font-medium text-neutral-700 dark:text-neutral-300 tabular-nums">
                  {formatNumber(item.total_barang)}
                </td>

                <!-- Total Harga Jual -->
                <td class="px-4 py-3 text-right font-mono font-bold text-neutral-900 dark:text-neutral-100 tabular-nums">
                  {formatRupiah(item.total_harga_jual)}
                </td>

                <!-- Laba Kotor -->
                <td class="px-4 py-3 text-right font-mono text-emerald-600 dark:text-emerald-400 tabular-nums font-medium">
                  +{formatRupiah(laba)}
                </td>
    ...
    <div class="px-4 py-3 border-t border-neutral-200 dark:border-neutral-800 bg-[var(--bg-subtle)]/60 flex items-center justify-between text-xs text-neutral-500">
      <span>Menampilkan {formatNumber(filteredList.length)} dari {formatNumber(penjualanList.length)} total transaksi</span>
      <span class="font-mono font-medium text-neutral-800 dark:text-neutral-200 tabular-nums">
        Akumulasi: {formatRupiah(totalOmzet)}
      </span>
    </div>
  </div>
</div>
